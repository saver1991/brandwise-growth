...
// We'll reuse the same data from the NewProfilePage
...
const EditProfilePage = () => {
  ...
  const { availableProfiles, addProfile, updateProfile } = useProfile();
    ...
    if (id) {
      ...
      if (profile) {
        ...
        setProfileForm({
          ...
          selectedTags: profile.tags.map(tag => tag.label)
        });
        setSelectedIntegrations(profile.integrations || []);
        
        // Set connection states for existing integrations
        const states: Record<string, "idle" | "connecting" | "connected"> = {};
        (profile.integrations || []).forEach(integration => {
          states[integration] = "connected";
        });
  ...
  const renderStepContent = () => {
    switch(currentStep) {
      ...
      case "review":
        return (
          <div className="space-y-6">
            <h3 className="text-lg font-medium">Review Your Updated Profile</h3>
    ...
    <div className="flex min-h-screen flex-col">
      ...
      <div className="container max-w-3xl mx-auto px-4 py-12">
        <div className="mb-8">
          ...
          <p className="text-muted-foreground mt-2">Update your profile details and integrations</p>
        </div>
